    ...
    def __getitem__(self, idx):
        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                elif self.alg == 'conmatch':
                    return idx, img_w, self.strong_transform(img), target
                elif self.alg == 'conmatch_3':
                    if target is None:
                        return idx, img_w, self.strong_transform(img), self.strong_transform(img), self.strong_transform(img)                        
                    else:
                        return idx, img_w, self.strong_transform(img), self.strong_transform(img), target
