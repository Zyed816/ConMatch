...
def get_optimizer_con(net, optim_name='SGD', lr=0.1, lr_con=0.1, momentum=0.9, weight_decay=0, nesterov=True, bn_wd_skip=True):
    '''
    return optimizer (name) in torch.optim.
    If bn_wd_skip, the optimizer does not apply
    weight decay regularization on parameters in batch normalization.
    ...
    # param_con = [param for name, param in net[1].named_parameters()]
    # param_backbone = [param for name, param in net[0].named_parameters()]
    ...
    for name, param in net.con_estimator.named_parameters():
        if ('bn' in name or 'bias' in name) and bn_wd_skip:
            no_decay_con.append(param)
        else:
            decay_con.append(param)
...
class EMA:
    ...
    def register(self):
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                self.shadow[name] = param.data.clone()
    #save 후에 ema_model을 업데이트 하기 위해서
    def update(self):
        for name, param in self.model.named_parameters():
            if param.requires_grad:
    ...
    #backup에는 param을 저장. 나중에 이 backup은 ema_model save에 사용됨.
